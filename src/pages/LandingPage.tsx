...
const fragmentShader = `
  ...
  void main() {
    ...
    // Color shifting based on normal, position, and time
    vec3 color1 = vec3(0.0, 0.96, 0.83); // Teal #00F5D4
    vec3 color2 = vec3(0.44, 0.0, 1.0); // Violet #7000FF
    vec3 color3 = vec3(1.0, 0.0, 0.9);   // Pink #FF00E5
...
const IridescentCore = () => {
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      ...
      // Pulse scale every ~400ms (1 beat = 0.4s -> frequency 2.5Hz -> roughly sin(time * 2.5 * PI * 2))
...
export const LandingPage = () => {
  ...
  // --- 3. Holographic Prism Scroll (GSAP) ---
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Animate each section as a face of a rotating prism
      sections.forEach((section, i) => {
        ...
        // The previous section rotates away
        ...
        // The current section rotates in
        gsap.to(section, {
          rotationX: 0,
          autoAlpha: 1,
          yPercent: 0,
  ...
  return (
    <div className="bg-[#020205] text-white selection:bg-primary/40 font-sans" style={{ perspective: '2000px' }}>
      
      {/* High-Frequency Scanlines Overlay */}
      <div className="fixed inset-0 z-[999] pointer-events-none opacity-[0.03] mix-blend-overlay
        bg-[linear-gradient(transparent_50%,_rgba(0,0,0,1)_50%)] bg-[length:100%_4px]" 
      />
      ...
      {/* NAVIGATION (Fixed outside scroll sequence) */}
      ...
      <div ref={containerRef} className="h-screen w-full relative overflow-hidden" style={{ transformStyle: 'preserve-3d' }}>
        
        {/* --- SECTION 1: HERO (Iridescent Core) --- */}
        <div ref={el => { if (el) sectionsRef.current[0] = el }} className="absolute inset-0 flex items-center justify-center pt-20 px-6 backface-hidden">
          {/* 3D Canvas Background */}
          <div className="absolute inset-0 z-0 opacity-80 mix-blend-screen pointer-events-none pt-32">
          ...
          <div className="relative z-10 text-center space-y-16 mt-32 max-w-7xl w-full">
        ...
        <div ref={el => { if (el) sectionsRef.current[1] = el }} className="absolute inset-0 flex items-center justify-center px-8 bg-[#020205] backface-hidden">
           ...
           <div className="w-full max-w-7xl pt-24">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-12 perspective-[2000px]">
                <TiltCard 
                  ...
                  desc="Your entire curriculum visualized as a glowing constellation of interconnected nodes via Llama 3.3 struct parsing." 
                  delay={0} 
                />
                <TiltCard 
                  icon={Cpu} 
                  title="Groq-Speed AI" 
                  desc="Sub-second response times for syllabus analysis and dynamic 6-month roadmap generation. Zero lag computation." 
                  delay={0.2} 
                />
                <TiltCard 
                  icon={BrainCircuit} 
                  title="Vision Grader" 
                  desc="Grade handwritten long-form answers instantly with Llama 4 Scout multimodal vision. Unmatched OCR precision." 
                  delay={0.4} 
                />
             </div>
           </div>
        ...
        <div ref={el => { if (el) sectionsRef.current[2] = el }} className="absolute inset-0 flex items-center justify-center px-8 bg-[#010103] backface-hidden">
      ...
      {/* FOOTER METADATA (Static at bottom) */}
